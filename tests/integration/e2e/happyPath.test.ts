...
describe('Happy path: RECEIVED → DONE', () => {
  it('POST /v1/intents — creates intent', async () => {
    const res = await app.inject({
      method: 'POST',
      url: '/v1/intents',
      headers: { 'content-type': 'application/json', 'x-idempotency-key': 'hp-idem-1' },
      body: JSON.stringify({ userId: 'user-demo', query: 'Sony WH-1000XM5', maxBudget: 30000, currency: 'gbp' }),
    });
  ...
  it('POST /v1/approvals/:id/decision — user approves', async () => {
    const res = await app.inject({
      ...
      headers: { 'content-type': 'application/json', 'x-idempotency-key': 'hp-approval-1' },
  ...
  it('GET /v1/intents/:intentId — returns intent', async () => {
    const res = await app.inject({ method: 'GET', url: `/v1/intents/${intentId}` });
    expect(res.statusCode).toBe(200);
